...
echo "=== STAGE 6: Cleaning up Infrastructure ==="
...
LB_IP=$(gcloud dns record-sets list --zone=gke-demo-zone --name="app.demo.gke." --type=A --format="value(rrdatas[0])" || true)
if [[ -n "$LB_IP" ]]; then
    gcloud dns record-sets transaction remove "$LB_IP" --name="app.demo.gke." --ttl=60 --type=A --zone=gke-demo-zone || true
